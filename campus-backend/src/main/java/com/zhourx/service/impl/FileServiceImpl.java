...
import com.zhourx.utils.FileAliOssUtils;
import com.zhourx.utils.FileMinioUtils;
...
public class FileServiceImpl implements FileService {
    ...
    @Autowired
    private FileMinioUtils minioUtils;
    ...
    // 上传头像功能minio版
    @Override
    public String uploadFileAvatarToMinio(MultipartFile file) {
        // 工具类获取值
        String bucketName = "avatarUrls";
        InputStream inputStream = null;
        try {
            // 判断文件大小
            long fileSize = file.getSize();
            ...
            // 判断文件是否存在
            boolean exists = minioUtils.checkFileIsExist(bucketName+"/"+fileName);
            if (exists) {
                // 如果文件已存在，则先删除原来的文件再进行覆盖
            ...
            // 参数2： 上传到oss文件路径和文件名称 /aa/bb/1.jpg
            // 参数3： 上传文件的输入流
            return minioUtils.upload(file,bucketName+"/"+fileName);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
