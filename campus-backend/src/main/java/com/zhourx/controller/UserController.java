...
import com.zhourx.annotation.AuthCheck;
...
import com.zhourx.constants.UserConstants;
...
import com.zhourx.utils.EMailUtils;
import com.zhourx.utils.SMSUtils;
import com.zhourx.utils.ValidateCodeUtils;
...
import static com.zhourx.constants.SystemConstants.PAGE_SIZE;
...
@RestController
@RequestMapping("/user")
@CrossOrigin(originPatterns = {"http://localhost:5173", "http://localhost:5174","http://47.121.118.209"}, allowCredentials = "true")
@Slf4j
@Api(tags = "用户管理模块")
public class UserController {
    ...
    public BaseResponse<String> sendPhoneUpdateMessage(String phone, HttpServletRequest request) {
        ...
        System.out.println(code);
        emailUtils.sendText("修改手机号","你本次的验证码为:"+code,loginUser.getEmail());
        // SMSUtils.sendMessage(phone, String.valueOf(code));
        return ResultUtils.success("短信发送成功");
    }
    ...
    public BaseResponse<String> sendMailUpdateMessage(String email, HttpServletRequest request) throws MessagingException {
        ...
        emailUtils.sendText("修改邮箱","你本次的验证码为:"+code,loginUser.getEmail());
    ...
    public BaseResponse<String> userRegister(@RequestBody UserRegisterRequest userRegisterRequest, HttpServletRequest request) {
        ...
        if (StringUtils.isAnyBlank(phone,email, account, password, checkPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "信息不全");
        }
        long userId = userService.userRegister(phone,email, account, password, checkPassword);
    ...
    @ApiImplicitParams(
            {@ApiImplicitParam(name = "phone", value = "手机号")})
    public BaseResponse<String> getUserByPhone(String phone) throws MessagingException {
        if (phone == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        ...
        if (user == null) {
            ...
            System.out.println(code);
            emailUtils.sendText("本次修改密码的验证码为:",""+code,user.getEmail());
     ...
     * 忘记密码
     *
     * @param forgetPasswordRequest 忘记密码请求
     * @return {@link BaseResponse}<{@link String}>
     */
    ...
    @ApiImplicitParams(
            {@ApiImplicitParam(name = "updatePasswordRequest", value = "修改密码请求")})
    public BaseResponse<String> updatePassword(@RequestBody ForgetPasswordRequest forgetPasswordRequest) {
        String phone = forgetPasswordRequest.getPhone();
        String password = forgetPasswordRequest.getPassword();
        String confirmPassword = forgetPasswordRequest.getConfirmPassword();
        if (StringUtils.isAnyBlank(phone, password, confirmPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        userService.updatePassword(phone, password, confirmPassword);
    ...
    public BaseResponse<String> updateUser(@RequestBody UserUpdateRequest updateRequest, HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(updateRequest.getEmail()) || StringUtils.isNotBlank(updateRequest.getPhone())) {
            if (StringUtils.isBlank(updateRequest.getCode())) {
                ...
                stringRedisTemplate.opsForValue().set(RedisConstants.LOGIN_USER_KEY + token,new Gson().toJson(user));
                stringRedisTemplate.expire(RedisConstants.LOGIN_USER_KEY + token, Duration.ofMinutes(15));
                return ResultUtils.success("ok");
            }
        }
